-- This database called HR manages the HR data of the small businesses.
...
-- Employees: 40 (rows) / Dependents: 30 / Departments: 11 / Jobs: 11 / Locations: 7 / Countries: 25 / Regions: 4

-- The initial resources (tables and tables' entries) come from https://www.sqltutorial.org/sql-sample-database/ .

DROP DATABASE HR_project;
...
INSERT INTO employees(employee_id,first_name,last_name,email,phone_number,hire_date,job_id,salary,manager_id,department_id) VALUES (103,'Alexander','Hunold','alexander.hunold@sqltutorial.org','590.423.4567','2018-01-03',9,9000.00,102,6);
INSERT INTO employees(employee_id,first_name,last_name,email,phone_number,hire_date,job_id,salary,manager_id,department_id) VALUES (104,'Bruce','Ernst','bruce.ernst@sqltutorial.org','590.423.4568','2022-05-21',9,6000.00,103,6);
INSERT INTO employees(employee_id,first_name,last_name,email,phone_number,hire_date,job_id,salary,manager_id,department_id) VALUES (105,'David','Austin','david.austin@sqltutorial.org','590.423.4569','2020-06-25',9,4800.00,103,6);
...
INSERT INTO employees(employee_id,first_name,last_name,email,phone_number,hire_date,job_id,salary,manager_id,department_id) VALUES (113,'Luis','Popp','luis.popp@sqltutorial.org','515.124.4567','2020-12-07',6,6900.00,108,10);
...
INSERT INTO employees(employee_id,first_name,last_name,email,phone_number,hire_date,job_id,salary,manager_id,department_id) VALUES (116,'Shelli','Baida','shelli.baida@sqltutorial.org','515.127.4563','2020-12-24',13,2900.00,114,3);
INSERT INTO employees(employee_id,first_name,last_name,email,phone_number,hire_date,job_id,salary,manager_id,department_id) VALUES (117,'Sigal','Tobias','sigal.tobias@sqltutorial.org','515.127.4564','2022-07-24',13,2800.00,114,3);
INSERT INTO employees(employee_id,first_name,last_name,email,phone_number,hire_date,job_id,salary,manager_id,department_id) VALUES (118,'Guy','Himuro','guy.himuro@sqltutorial.org','515.127.4565','2022-11-15',13,2600.00,114,3);
...
INSERT INTO employees(employee_id,first_name,last_name,email,phone_number,hire_date,job_id,salary,manager_id,department_id) VALUES (120,'Matthew','Weiss','matthew.weiss@sqltutorial.org','650.123.1234','2019-07-18',19,5000.00,100,5);
...
INSERT INTO employees(employee_id,first_name,last_name,email,phone_number,hire_date,job_id,salary,manager_id,department_id) VALUES (123,'Shanta','Vollman','shanta.vollman@sqltutorial.org','650.123.4234','2018-10-10',19,6500.00,100,5);
...
INSERT INTO employees(employee_id,first_name,last_name,email,phone_number,hire_date,job_id,salary,manager_id,department_id) VALUES (176,'Jonathon','Taylor','jonathon.taylor@sqltutorial.org',NULL,'2018-03-24',16,8600.00,100,8);
...
INSERT INTO employees(employee_id,first_name,last_name,email,phone_number,hire_date,job_id,salary,manager_id,department_id) VALUES (192,'Sarah','Bell','sarah.bell@sqltutorial.org','650.501.1876','2019-02-04',17,4000.00,123,5);
...
INSERT INTO employees(employee_id,first_name,last_name,email,phone_number,hire_date,job_id,salary,manager_id,department_id) VALUES (202,'Pat','Fay','pat.fay@sqltutorial.org','603.123.6666','2015-08-17',11,3000.00,201,2);
...
INSERT INTO employees(employee_id,first_name,last_name,email,phone_number,hire_date,job_id,salary,manager_id,department_id) VALUES (204,'Hermann','Baer','hermann.baer@sqltutorial.org','515.123.8888','1977-06-07',12,10000.00,101,7);
...
INSERT INTO employees(employee_id,first_name,last_name,email,phone_number,hire_date,job_id,salary,manager_id,department_id) VALUES (206,'William','Gietz','william.gietz@sqltutorial.org','515.123.8181','2000-06-07',1,8300.00,205,11);
...
RENAME COLUMN first_name to first_name_d,
RENAME COLUMN last_name to last_name_d;

SELECT * FROM dependents;


-- presentation of company's locations worldwide

CREATE VIEW regions_countries_locations AS
SELECT regions.region_id, region_name, countries.country_id, country_name, location_id, state_province, city, postal_code, street_address 
FROM countries
RIGHT JOIN locations ON locations.country_id = countries.country_id
RIGHT JOIN regions ON regions.region_id = countries.region_id
WHERE location_id IS NOT NULL
ORDER BY region_id;

SELECT * FROM regions_countries_locations;

-- getting the number of employees per region

SELECT 
region_name, 
COUNT(*) AS number_of_employees 
FROM departments d
JOIN regions_countries_locations rcd ON rcd.location_id = d.location_id
JOIN employees e ON e.department_id = d.department_id
GROUP BY region_name 
ORDER BY number_of_employees DESC;


-- getting the number of employees per country & city
...
-- getting the number of employees per city

SELECT 
country_name, country_id, city, 
...
-- details per Seattle (18 employees and various departments)
...
HAVING city IN ('Seattle')
ORDER BY employees_number DESC;
 
 

-- view: departments_employees_jobs

CREATE VIEW departments_employees_jobs AS
SELECT employee_id, first_name, last_name, email, phone_number, hire_date, j.job_id, salary, manager_id, d.department_id, job_title, min_salary, max_salary, department_name, location_id  
FROM employees e
JOIN jobs j ON j.job_id = e.job_id
JOIN departments d ON d.department_id = e.department_id
ORDER BY employee_id;

SELECT * FROM departments_employees_jobs;

-- getting the total number of job roles in the company

SELECT COUNT(DISTINCT job_title) FROM departments_employees_jobs;

-- listing job roles in the company

SELECT 
job_title, 
COUNT(*) AS total_employees
FROM departments_employees_jobs
GROUP BY job_title
ORDER BY total_employees DESC;

-- getting the overview of 10 maximum/ minimum salaries per job_title (general guidelines)
...
min_salary AS salary_floor, 
max_salary AS salary_ceiling,
CASE
WHEN (MIN(salary) BETWEEN min_salary and max_salary AND MAX(salary) BETWEEN min_salary and max_salary) = 1 THEN 'yes'
ELSE 'no'
END AS is_in_range 
FROM departments_employees_jobs
GROUP BY job_title, salary_floor, salary_ceiling
...
-- getting the avg, sum of salaries, count of staff per department
SELECT
DISTINCT department_id, 
department_name, 
ROUND(AVG(salary) OVER (PARTITION BY department_name),0) AS AVG_salary_per_dept,
ROUND(SUM(salary) OVER (PARTITION BY department_name),0) AS SUM_of_salaries_per_dept,
ROUND(SUM(salary) OVER(),0) AS total_payroll,
COUNT(*) OVER(PARTITION BY department_name) as COUNT_of_staff_per_dept,
COUNT(*) OVER() AS total_staff
FROM departments_employees_jobs
ORDER BY department_id;

-- getting 10 best paid employees in the company
SELECT
employee_id, 
first_name,
last_name,
department_name, 
salary,
RANK() OVER(PARTITION BY department_name ORDER BY salary DESC) AS salary_department_rank,
RANK() OVER(ORDER BY salary DESC) AS salary_company_rank
FROM departments_employees_jobs
ORDER BY salary_company_rank
LIMIT 10;

-- getting 1 best paid employee per department
SELECT 
COUNT(DISTINCT department_name) AS number_of_depts
FROM departments_employees_jobs;
...
RANK() OVER(ORDER BY salary DESC) AS salary_company_rank
FROM departments_employees_jobs
ORDER BY salary_department_rank
LIMIT 11;


-- calculation of seniority and salaries
CREATE VIEW employee_seniority AS
SELECT 
employee_id,
first_name,
Last_name,
hire_date,
ROUND(DATEDIFF(CURDATE(), hire_date)/365,0) AS years_worked,
CASE
WHEN ROUND(DATEDIFF(CURDATE(), hire_date)/365,0) BETWEEN 0 AND 2 THEN 'junior'
WHEN ROUND(DATEDIFF(CURDATE(), hire_date)/365,0) BETWEEN 3 AND 5 THEN 'mid'
WHEN ROUND(DATEDIFF(CURDATE(), hire_date)/365,0) > 5 THEN 'senior'
END AS seniority  
FROM departments_employees_jobs;
...
JOIN departments_employees_jobs dej ON dej.employee_id = es.employee_id
GROUP BY seniority
...
SELECT 
region_name,
ROUND(SUM(salary),0) AS salaries_total, 
COUNT(employee_id) AS total_staff,
ROUND(SUM(salary)/COUNT(employee_id),0) AS avg_salary_per_employee
FROM departments_employees_jobs dej
JOIN regions_countries_locations rcl ON rcl.location_id = dej.location_id
GROUP BY region_name;



-- select employees who have higher salary than the average per region (Americas) (subquery example)
SELECT 
employee_id,
first_name,
last_name,
...
CREATE VIEW salary_higher_than_average_Americas AS 
SELECT 
employee_id,
first_name,
last_name,
...
SELECT * FROM salary_higher_than_average_Americas;

-- how many people in Americas have salaries higher than an average per region
SELECT COUNT(*) FROM salary_higher_than_average_Americas;

-- select employees who have higher salary than the average per region (Europe) (subquery example)
SELECT 
employee_id,
first_name,
last_name,
...
CREATE VIEW salary_higher_than_average_Europe AS
SELECT 
employee_id,
first_name,
last_name,
...
-- how many people in Europe have salaries higher than an average per region
SELECT COUNT(*) FROM salary_higher_than_average_Europe;


-- creating one big metatable for all data to be able to export for the reason of google sheets dashboard
-- view: regions_countries_locations
-- view: departments_employees_jobs


SELECT * 
FROM departments_employees_jobs dej
LEFT JOIN regions_countries_locations rcl ON rcl.location_id = dej.location_id
LEFT JOIN dependents d ON d.employee_id = dej.employee_id;


-- looking for missing values, empty cells etc. in the dependent table
SELECT * FROM dependents;

SELECT * FROM dependents 
WHERE dependent_id IS NULL OR first_name_d IS NULL OR last_name_d IS NULL OR relationship IS NULL;
